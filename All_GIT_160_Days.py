...
def add_chart_and_metrics(project_df, project_name):
    ...
    project_df.plot(x='Created Date', y='Time in Progress (hours)', figsize=(6, 4))
    ...
    project_df.plot(x='Created Date', y='Time to Close (hours)', figsize=(6, 4))
...
jql_query = 'project in (INFRA, PSY, DATA, OPS) AND issuetype in (Bug, Story, Task, Sub-task, Improvement, Investigation, "New Feature", Request) AND status = Done AND created > -160d'
